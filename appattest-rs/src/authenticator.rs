...
impl AuthenticatorData {
    ...
    pub(crate) fn is_valid_aaguid(&self, allow_development_env: bool) -> bool {
        if let Some(aaguid) = &self.aaguid {
            let id_bytes = aaguid.bytes();
            let prod_id = APP_ATTEST.as_bytes();
            let dev_id  = APP_ATTEST_DEVELOP.as_bytes();

            if id_bytes == prod_id {
                return true;
            }
            if id_bytes == dev_id {
                return allow_development_env;
            }
        }
        false
    }
